(function() {
  ...
  TaskDetaliController.$inject = ['$state','TaskDetali','entity','paginationConstants'];
  /** @ngInject */
  function TaskDetaliController( $state, TaskDetali,entity, paginationConstants) {
    var vm = this;
    vm.page = 1;
    vm.totalItems = null;
    vm.loadAll = loadAll;
    ...
    vm.jobs=[];
    function loadAll() {
      TaskDetali.TaskDetaliFound().query({
        page: vm.page,
        pagesize: paginationConstants.itemsPerPage
      }, onSuccess, onError);
    }
    function resumeJobs(){
      TaskDetali.TaskDetaliResume().update(
        vm.selectedTags );
    }
    function delJobs(){
      TaskDetali.TaskDetaliDelete().update(
    ...
    function pauseJobs(){
      TaskDetali.TaskDetaliPause().update(
        vm.selectedTags );
    }
    function loadJobs(){
